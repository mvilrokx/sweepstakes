...
const insertTournament = (knex, Promise, tournament) => {
  return knex('tournaments').returning('id').insert({
    ...
    return Promise.map(Object.keys(tournament.groups), (groupName) => {
      return knex('tournament_groups').returning('id').insert({
        ...
        console.log(`Successfully inserted Group ${groupName} into Tournament ${tournament.name}`)
        ...
        if (error.code === '23505' && error.constraint === 'uniqueTournamentGroup') {
  ...
  }).catch((error) => { // TODO: This should still trigger inserting Groups and then participants

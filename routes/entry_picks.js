        ...
        .then((tournamentGroups) => {
          ...
          entryPickQueries.getEntryPicks(req.user, req.params.entry_id)
            .then((picks) => {
              ...
              res.render('new_pick', { isLoggedIn: req.isAuthenticated(), user: req.user, tournamentGroups: tournamentGroups, entry: userEntry[0], existing_picks: picks})
            })
        })
    })
    .catch((error) => {
...
router.post('/:entry_id/picks', isLoggedIn, (req, res, next) => {
  entryPickQueries.insertEntryPick({user: req.user, entry_id: req.params.entry_id, tournament_participant_id: req.body.participant_id})
    .then(() => {
